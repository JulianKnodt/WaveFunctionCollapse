...
fn run() {
  ...
  out
    .save(format!("{}_{}_{}.png", folder, w, h))
...
fn knot_description() -> Desc<&'static str, bool> {
  ...
  let corner_tile_desc = TileDesc::from((360, vec![
    (true, vec![DEG_0, DEG_270]),
    (false, vec![DEG_180, DEG_90]),
  ]));
  let empty_tile_desc = TileDesc::from((90, vec![(false, vec![DEG_0])]));
  let t_tile_desc = TileDesc::from((360, vec![
    (true, vec![DEG_0, DEG_180, DEG_90]),
    (false, vec![DEG_270]),
  ]));
...
fn circuit_description() -> Desc<&'static str, usize> {
  ...
  let connection = TileDesc::from((360, vec![
    (3, vec![DEG_0]),
    (2, vec![DEG_90]),
    (4, vec![DEG_180]),
    (1, vec![DEG_270]),
  ]));
  let corner = TileDesc::from((360, vec![
    (5, vec![DEG_0, DEG_270]),
    (4, vec![DEG_180]),
    (3, vec![DEG_90]),
  ]));
  let dskew = TileDesc::from((180, vec![(1, vec![DEG_0, DEG_90])]));
  let skew = TileDesc::from((360, vec![
    (1, vec![DEG_0, DEG_270]),
    (5, vec![DEG_90, DEG_180]),
  ]));
  let substrate = TileDesc::from((90, vec![(5, vec![DEG_0])]));
  let t = TileDesc::from((360, vec![
    (1, vec![DEG_0, DEG_90, DEG_180]),
    (5, vec![DEG_270]),
  ]));
  let track = TileDesc::from((180, vec![(5, vec![DEG_0]), (1, vec![DEG_90])]));
  let transition = TileDesc::from((360, vec![
    (5, vec![DEG_0, DEG_180]),
    (1, vec![DEG_90]),
    (0, vec![DEG_270]),
  ]));
  let turn = TileDesc::from((360, vec![
    (1, vec![DEG_0, DEG_270]),
    (5, vec![DEG_90, DEG_180]),
  ]));
  let viad = TileDesc::from((180, vec![(1, vec![DEG_0]), (5, vec![DEG_90])]));
  let vias = TileDesc::from((360, vec![
    (1, vec![DEG_270]),
    (5, vec![DEG_0, DEG_90, DEG_180]),
  ]));
